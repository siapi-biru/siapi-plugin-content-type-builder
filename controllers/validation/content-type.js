...
const { formatYupErrors, nameToSlug } = require('siapi-utils');
...
const forbiddenContentTypeNameValidator = () => {
  const reservedNames = siapi.plugins['content-type-builder'].services.builder.getReservedNames()
    .models;
...
const alreadyUsedContentTypeName = isEdition => {
  const usedNames = Object.values(siapi.contentTypes).map(ct => ct.modelName);
