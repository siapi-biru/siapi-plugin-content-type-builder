...
const utils = require('siapi-utils');
const { isMediaAttribute } = require('siapi-utils').contentTypes;

const toUID = (name, plugin) => {
  const modelUID = Object.keys(siapi.contentTypes).find(key => {
    const ct = siapi.contentTypes[key];
  ...
  if (isMediaAttribute(attribute)) {
    ...
    return {
      ...
      targetColumnName: _.get(
        siapi.getModel(targetEntity, plugin),
  ...
  return {
    ...
    attributes: Object.keys(schema.attributes).reduce((acc, key) => {
      ...
      if (attr.type === 'component') {
        ...
        if (!_.has(siapi.components, attr.component)) {
      ...
      if (
        ...
        acc[key] = {
          ...
          components: attr.components.map(value => {
            ...
            if (!_.has(siapi.components, value)) {
              throw new Error('component.notFound');
            }
