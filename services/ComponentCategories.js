...
const editCategory = async (name, infos) => {
  ...
  if (!categoryExists(name)) {
    throw siapi.errors.notFound('cateogry.notFound');
  }

  if (categoryExists(newName)) {
    throw siapi.errors.badRequest('Name already taken');
  }
...
const categoryExists = name => {
  const matchingIndex = Object.values(siapi.components).findIndex(
