...
module.exports = function createBuilder() {
  const components = Object.keys(siapi.components).map(key => {
    const compo = siapi.components[key];
    ...
    return {
      ...
      dir: path.join(siapi.dir, 'components', compo.category),
      schema: compo.__schema__,
    };
  });

  const contentTypes = Object.keys(siapi.contentTypes).map(key => {
    const contentType = siapi.contentTypes[key];
  ...
  return {
      ...
      return Object.keys(attributes).reduce((acc, key) => {
        ...
        if (_.has(attribute, 'type')) {
          if (attribute.type === 'media') {
            const fileModel = siapi.getModel('file', 'upload');
    ...
    writeFiles() {
      return Promise.all(
        ...
        .catch(error => {
          siapi.log.error('Error writing schema files');
