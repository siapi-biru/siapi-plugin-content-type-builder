...
async function clear(uid) {
  const { apiName, __filename__ } = siapi.contentTypes[uid];

  const apiFolder = path.join(siapi.dir, 'api', apiName);
...
async function backup(uid) {
  const { apiName } = siapi.contentTypes[uid];

  const apiFolder = path.join(siapi.dir, 'api', apiName);
  const backupFolder = path.join(siapi.dir, 'api', '.backup', apiName);
...
async function deleteBackup(uid) {
  ...
  const backupFolder = path.join(siapi.dir, 'api', '.backup');
  const apiBackupFolder = path.join(siapi.dir, 'api', '.backup', apiName);
